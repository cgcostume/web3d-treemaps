...
//                      texture.minFilter = THREE.LinearFilter;
	...
	var ClassicGeometry = function() {
        ...
        this.mode = THREE.TrianglesDrawMode;
	};
	...
	ClassicGeometry.prototype.checkFinished = function() {
		if((this.indicesLoaded || this.indexArray) && this.loadedAttributes === this.totalAttributes) {
	...
	Mesh.prototype.attachToNode = function(threeNode) {
		...
		this.primitives.forEach(function(primitive) {
			...
			if (!that.skin) {
                ...
                threeMesh.setDrawMode(primitive.geometry.mode);
		...
		handleMesh: {
			value: function(entryID, description, userInfo) {
				...
				for (var i = 0 ; i < primitivesDescription.length ; i++) {
					...
					if (primitiveDescription.mode === WebGLRenderingContext.TRIANGLES || primitiveDescription.mode === WebGLRenderingContext.TRIANGLE_STRIP) {

						var geometry = new ClassicGeometry();
                        
                        if (primitiveDescription.mode === WebGLRenderingContext.TRIANGLE_STRIP)
                        {
                            geometry.mode = THREE.TriangleStripDrawMode;
                        }
                        ...
                        if (primitiveDescription.indices) {
                            geometry.indicesLoaded = false;
